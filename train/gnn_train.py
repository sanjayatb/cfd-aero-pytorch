...
from data.ahmed_ml_dataset import AhmedMLGNNDataset
from model.gnn import DragGNN_XL

# Configuration dictionary to hold hyperparameters and settings
config = {
    'cuda': True,
    'exp_name': 'DragPrediction_AhmedML_DragGNN_10epochs_NeurIPS',
    'seed': 1,
    'batch_size': 1,
    'epochs': 10,
    'lr': 0.001,
    'optimizer': 'adam',
    'dataset_path': '../../AhmedML/all/stl',  # Update this with your dataset path
    'aero_coeff': '../../AhmedML/all/force_mom_all.csv',
...
def get_dataloaders(dataset_path: str, aero_coeff: str, subset_dir: str, batch_size: int) -> tuple:
    ...
    full_dataset = AhmedMLGNNDataset(root_dir=dataset_path, csv_file=aero_coeff)
    ...
    train_dataloader = GeoDataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=16)
    val_dataloader = GeoDataLoader(val_dataset, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=16)
    test_dataloader = GeoDataLoader(test_dataset, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=16)
