...
from data.ahmed_ml_dataset import AhmedMLDataset
from model.pointnet import RegPointNet

# Configuration dictionary to hold hyperparameters and settings
config = {
    'exp_name': 'DragPrediction_AhemedML_PointNet_r2_batchsize32_10epochs_100kpoints_tsne_NeurIPS',
    'cuda': True,
    'seed': 1,
    'num_points': 1500,
    'lr': 0.001,
...
def test_model(model: torch.nn.Module, test_dataloader: DataLoader, config: dict):
    ...
    with torch.no_grad():
        for data, targets in test_dataloader:
            ...
            data, targets = data.to(device), targets.to(device).squeeze(dim=1)
            data = data.permute(0, 2, 1)
            outputs = model(data)
